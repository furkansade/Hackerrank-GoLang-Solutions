...
func countApplesAndOranges(s int32, t int32, a int32, b int32, apples []int32, oranges []int32) {
	...
	house_width := t - s
	house_area_apple := d_apple_tree + house_width
	house_area_orange := d_orange_tree - house_width

	var total_apple, total_orange int32

	for _, apple := range apples {
		if apple >= d_apple_tree && apple <= house_area_apple {
			total_apple++
		}
	}

	for _, orange := range oranges {
		if orange <= d_orange_tree && orange >= house_area_orange {
			total_orange++
		}
	}

	fmt.Println(total_apple)
	fmt.Println(total_orange)
}
